...
describe("without global prefix", () => {
  ...
  it("creates templates from input params", () => {
    ...
    expect(
      new Route("/user/:user/comments/:comment").create({ user: 1, comment: "something" }),
    ).toBe("/user/1/comments/something");
    expect(new Route("/user/:user/comments/:comment").create({ user: 2, comment: "else" })).toBe(
      "/user/2/comments/else",
    );
  });
});
...
describe("with global prefix", () => {
  ...
  it("supports global prefix", () => {
    ...
    expect(
      new Route("/user/:user/comments/:comment").create({ user: 1, comment: "something" }),
    ).toBe("https://example.com/user/1/comments/something");
  });
});
...
describe("with router", () => {
  ...
  it("supports global prefix", () => {
    ...
    expect(
      router.add("/user/:user/comments/:comment").create({ user: 1, comment: "something" }),
    ).toBe("https://example.com/user/1/comments/something");
  });
